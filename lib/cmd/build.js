                ...
                api('build', {
                    ...
                    if (response.token) {
                        ...
                        inquirer.prompt([{
                            ...
                            var valid = semver.valid(answers.version);
                            if (!valid) return util.error('invalid version build number. Expected x.x.x');
                            var old_version = manifest.version;
                            valid = semver.gte(answers.version, old_version);
                            if (!valid) return util.error("the version number provided (" + answers.version + ") cannot be lower than current version number (" + manifest.version + ")");
                            ...
                            fs.writeFile(global.PROJECT_HOME + sep + 'app.manifest', JSON.stringify(manifest, null, 4), function (e) {
                                ...
                                nodegit.Repository.open(path.resolve(dir, ".git")).then(function (repo) {
                                    ...
                                    return repository.createCommit("HEAD", author, committer,
                                        ...
                                        var caption = "";
                                        if (args.indexOf('--production') > -1) caption = '\tProduction build #' + build + ' (' + manifest.version + '.' + build + ')';
                                        if (args.indexOf('--prod') > -1) caption = '\tProduction build #' + build + ' (' + manifest.version + '.' + build + ')';
                                        if (args.indexOf('--alpha') > -1) caption = '\tAlpha build #' + build + ' (' + manifest.version + '.' + build + ')';
                                        if (args.indexOf('--beta') > -1) caption = '\tBeta build #' + build + ' (' + manifest.version + '.' + build + ')';
                                        if (caption == "") caption = '\tProduction build #' + build + ' (' + manifest.version + '.' + build + ')';
                                    ...
                                    nodegit.Remote.create(repository, "origin",
                                        ...
                                        nodegit.Remote.delete(repository, "origin").then(function (res) {
                                            nodegit.Remote.create(repository, "origin",
                                                ...
                                                console.log('');
                                                var branch = 1;
                                                if (args.indexOf('--production') > -1) branch = 1;
                                                if (args.indexOf('--prod') > -1) branch = 1;
                                                if (args.indexOf('--alpha') > -1) branch = 2;
                                                if (args.indexOf('--beta') > -1) branch = 3;
                                                ...
                                                api('build/id', {
                                                    ...
                                                    if (r.status == "OK") {
                                                        ...
                                                        console.log('You can monitor the build progress at https://console.omneedia.com/\n');
                                                    } else {
