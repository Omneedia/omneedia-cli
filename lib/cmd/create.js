module.exports = function () {
    ...
    fs.stat(PROJECT_HOME + PROJECT_ROOT + sep + PROJECT_NAME + sep + 'app.manifest', function (e, r) {
        ...
        console.log(chalk.bold(' Create package ' + PROJECT_NAME + '\n'));
                    ...
                    fs.mkdir(require('os').tmpdir + sep + sid, function () {
                        ...
                        readStream.pipe(unzip.Parse()).pipe(writeStream.on('close', function () {
                            fs.readdir(require('os').tmpdir + sep + sid, function (e, list) {
                                ...
                                fs.move(require('os').tmpdir + sep + sid + sep + list[0], dir + sep, {
                                    overwrite: true
                                }, function (e) {
                                    if (e) util.error('Failed!');
                                    ...
                                    fs.readFile(global.PROJECT_HOME + sep + 'app.manifest', function (e, r) {
                                        ...
                                        settings.update(manifest, function (settings) {
                                            fs.mkdir(global.PROJECT_ETC, function () {
                                                fs.writeFile(global.PROJECT_ETC + '/settings.json', settings, function (e) {
                                                    console.log('	' + emojic.whiteCheckMark + '   Settings updated.');
                                                    console.log('');
                                                    require('../update');
                                                });
                                            });
