module.exports = function() {
    ...
    function mysql_start() {
        if (!isWin) {
            ...
            shelljs.exec(userdirdata + path.sep + 'mysql.cmd', { silent: true });
            shelljs.exec("Wmic /output:\"" + pid + "\" process where (CommandLine like '%mysqld%') get Name,CommandLine,ProcessId", { silent: true });
    ...
    function mysql_update() {
        ...
        function update(settings, db, i, cb) {
            ...
            var sequelize = new Sequelize(setup, { operatorsAliases: false, define: { freezeTableName: true } });
            var dbo = db[i];
    ...
    function install_mysql() {
        ...
        function setmeup() {
            ...
            fs.createReadStream(filename).pipe(unzip.Extract({ path: path.dirname(filename) })).on('error', function() {
