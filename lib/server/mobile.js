module.exports = function(app, express) {
    ...
    app.get('/api/(*)', function(req, res) {
        ...
        var SCRIPT = [];
        var classname = req.url.split('/api/')[1].split('.')[0];
            ...
            loader(0, function() {
                ...
                for (var i = 0; i < PACK.class.length; i++) {
                    ...
                    if (req.url.indexOf('.js') > -1) {
                        SCRIPT.push('App.define("api.' + classname + '.' + PACK.class[i] + '",' + ' {');
                        SCRIPT.push('\statics: {');
                        SCRIPT.push('\tajax: function(o) {');
                    } else {
                    ...
                    for (var j = 0; j < statics.length; j++) {
                        ...
                        if (req.url.indexOf('.js') > -1)
                            SCRIPT.push('\t' + fn + ': function(' + args.join(',') + ') {');
                        else
                        ...
                        SCRIPT.push('\t\t\t\tif (typeof data[0].data==="string") return callback(data[0].data);');
                        SCRIPT.push('\t\t\t\tif (!typeof data[0].result) return callback(data[0]);');
    ...
    app.get('/mobile/qrcode', function(req, res) {
        ...
        //console.log(req.protocol + '://' + getIPAddress() + ':' + manifest.server.port + '/default');
    ...
    app.get('/cordova_plugins.js', function(req, res) {
        ...
        function add_plugin(d, i, cb) {
            if (!d[i]) return cb();
            var filename = plugins_dir + d[i] + sep + 'plugin.xml';
                    ...
                    for (var el in result.plugin) {
                                ...
                                if (item.$.name == os_type) {
                                    ...
                                    if (item['js-module']) {
                                        for (var x = 0; x < module.length; x++) {
                                            ...
                                            var clobbers = [];
                                            var merges = [];
                                            if (module[x].clobbers) {
                                                for (var k = 0; k < module[x].clobbers.length; k++) {
                                                    clobbers.push(module[x].clobbers[k].$.target);
                                                };
                                            };
                                            if (module[x].merges) {
                                                for (var k = 0; k < module[x].merges.length; k++) {
                                                    merges.push(module[x].merges[k].$.target);
                                                };
                                            };
                                            platform.push({
                        ...
                        if (el == "js-module") {
                            for (var j = 0; j < result.plugin['js-module'].length; j++) {
                                ...
                                if (result.plugin['js-module'][j].merges) {
                                    for (var k = 0; k < result.plugin['js-module'][j].merges.length; k++) {
                                        merges.push(result.plugin['js-module'][j].merges[k].$.target);
                                    };
                                };
                                jsmodule.push({
        ...
        add_plugin(d, 0, function() {
            ...
            html.push("\nmodule.exports.metadata = " + JSON.stringify(metadata, null, 14) + ";");
            html.push("});");
            res.end(html.join('\n'));
        });
    });
