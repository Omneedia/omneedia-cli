				...
				for (var i=0;i<manifest.api.length;i++) {
					if (manifest.api[i].indexOf(':')>-1) {
						...
						for (var z=0;z<settings.api.length;z++) {
							if (settings.api[z].name==manifest.api[i].split(':')[0]) api_to_create=1;	
						};
						if (api_to_create==-1) {
							settings.api.push({
								name: manifest.api[i].split(':')[0],
								uri: 'http(s)://api.server.com/',
								class: []
							});							
						};
						for (var z=0;z<settings.api.length;z++) {
							if (settings.api[z].name==manifest.api[i].split(':')[0]) {
								if (settings.api[z].class.indexOf(manifest.api[i].split(':')[1])==-1) settings.api[z].class.push(manifest.api[i].split(':')[1]);
							}
						}
					};
				};
